...
?>
	<table>
		...
		<tr>
			<td>Dibayar</td>
			<td>:
				<input id="b" type="text" class="noRek" name="dibayar" value="<?=$bayar?>" onMouseOver="$('b').select()" onChange="$('c').innerHTML=$('b').value-$('a').value"/> <!-- .select() fungsinya untuk memfokuskan pointer pada saat MouseOver -->
			</td>
		</tr>
		<tr>
			<td>Kembalian</td>
